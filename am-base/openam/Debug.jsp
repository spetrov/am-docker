...
<%
    ...
    if (!LoggerFactory.getILoggerFactory().getClass().getName().equals("org.forgerock.openam.slf4j.AMLoggerFactory")) {
%>
<p><%= resourceBundle.getString("not-using-legacy-debug") %></p>
<p><%= MessageFormat.format(resourceBundle.getString("logback-link"), "Logback.jsp") %></p>
</body></html>
<%
        return;
    }
    ...
    if (performAction) {
        ...
        if (samlDecryptionEnable != null) {
            ...
            Debug.getInstance(SAML2SDKUtils.class + ".decryptedAssertions").setDebug(enable ? Debug.MESSAGE : Debug.ERROR);
        }
    }
%>
...
<%
    ...
    if (category != null) {
        out.println(resourceBundle.getString("label-category") + " = " + ESAPI.encoder().encodeForHTML(category));
...
%>
                ...
                <form name="samldecryptiondebug" action="Debug.jsp" method="POST">
                    ...
                    <input type="hidden" name="formToken" value="<%= ESAPI.encoder().encodeForHTMLAttribute(formToken) %>" />
                    <input type="hidden" name="category" value="<%= ESAPI.encoder().encodeForHTMLAttribute(category) %>" />
                    <input type="hidden" name="level" value="<%= ESAPI.encoder().encodeForHTMLAttribute(level) %>" />
...
<%
        ...
        out.println("Instance" + " = " + ESAPI.encoder().encodeForHTML(instance));
    }
    out.println("<br />");
    ...
    if (!performAction || (performAction && (samlDecryptionEnable != null))) {
        ...
        if (category != null) {
            out.println("<input name='category' type='hidden' value='" + ESAPI.encoder().encodeForHTMLAttribute(category) + "' />");
        } else {
            out.println("<input name='instance' type='hidden' value='" + ESAPI.encoder().encodeForHTMLAttribute(instance) + "' />");
        } 
        out.println("<input name='level' type='hidden' value='" + levelint + "' />");
        out.println("<input name='do' type='hidden' value='true' />");
        out.println("<input type='hidden' name='formToken' value='" + ESAPI.encoder().encodeForHTMLAttribute(formToken) + "' />");
