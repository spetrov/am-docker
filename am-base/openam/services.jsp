 ...
 -t dynamic -a "iplanet-am-session-max-session-time=240"</pre>
            ...
            <%
                ...
                for (String serviceName : serviceNames) {
                    String encodedServiceName = ESAPI.encoder().encodeForHTML(serviceName);

                    out.println("<div class='service' name='" + encodedServiceName + "'>");
                    out.println("<div class='serviceName'>" + encodedServiceName + "</div>");
                    ...
                    if (ssm != null) {
                        ...
                        for (String schemaType : serviceSchemaList.keySet()) {
                            ...
                            if (ss != null) {
                                ...
                                out.println("<div class='schemaType'><div class='schemaTitle'>" + ESAPI.encoder().encodeForHTML(schemaType) + "</div>");
                                out.println("<div class='schemaBlock'>");

                                boolean rowAlternator = true;
                                for (String serviceAttributeName : serviceAttributeNames) {
                                    // Alternating row backgrounds
                                    rowAlternator = !rowAlternator;

                                    AttributeSchema as = ss.getAttributeSchema(serviceAttributeName);
                                    ...
                                    String encodedServiceAttributeName = ESAPI.encoder().encodeForHTML(serviceAttributeName);
                                    out.println("<div class='name'>" + encodedServiceAttributeName + "</div>");
                                    try {
                                        if (as.getI18NKey() != null)
                                            out.println("<div class='description'>" + ESAPI.encoder().encodeForHTML((bundle!=null ? bundle.getString(as.getI18NKey()) : as.getName())) + "</div>");
                                    } catch (MissingResourceException mre) {
                                        out.println("<div class='descriptionMissing'>" + encodedServiceAttributeName + " (missing description)</div>");
                                    }
                                    try {
                                        if (bundle != null && as.getI18NKey() != null)
                                            out.println("<div class='help'>" + ESAPI.encoder().encodeForHTML(bundle.getString(as.getI18NKey().concat(".help"))) + "</div>");
                                    } catch (MissingResourceException mre) {
                                    }
                                    out.println("</div>");
                                    out.println("</div>");
