...
<%
   ...
   try {
    ...
    buffer.append("productNameSrc=")
        .append(Uris.urlEncodeQueryParameterNameOrValue(productNameSrc))
        .append("&versionFile=")
	.append(Uris.urlEncodeQueryParameterNameOrValue(versionFile))
	.append("&productNameHeight=")
        .append(Uris.urlEncodeQueryParameterNameOrValue(productNameHeight))
        .append("&productNameWidth=")
        .append(Uris.urlEncodeQueryParameterNameOrValue(productNameWidth));
%>
...
<head>
  ...
  <meta name="Copyright" content="Copyright &copy; 2010-2021, ForgeRock AS. All Rights Reserved.">
  <cc:stylesheet />
</head>
...
<frameset rows="110,*,60" frameborder="no" border="0" framespacing="0">
  <frame src="<%= ESAPI.encoder().encodeForHTMLAttribute(buffer.toString()) %>" name="topFrame" scrolling="no" noresize="noresize" id="topFrame" title="Masthead Frame">
  <frame src="<%= ESAPI.encoder().encodeForHTMLAttribute(request.getContextPath() + "/base/Version") %>" name="mainFrame" id="mainFrame" title="Content Frame">
  <frame src="<%= ESAPI.encoder().encodeForHTMLAttribute(buttonBuffer.toString()) %>" name="buttonFrame" scrolling="no" noresize="noresize" id="bottomFrame" title="Button Frame">
  <noframes>
    <body>
