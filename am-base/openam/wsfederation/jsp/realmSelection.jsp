...
<%! private final Logger logger = LoggerFactory.getLogger("jsp.realmSelection"); %>
<%
    ...
    if (logger.isDebugEnabled()) {
        logger.debug(jspFile + "wreply: "+wreply);
    }
    ...
    if (logger.isDebugEnabled()) {
        logger.debug(jspFile + "Selected realm: " + selectedRealm);
    }
    if ( selectedRealm != null )
    {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(jspFile + "Redirecting to: "+url);
        }
...
<link rel="stylesheet" href="<%= ESAPI.encoder().encodeForHTMLAttribute(contextPath) %>/css/styles.css" type="text/css" />
<script language="JavaScript" src="<%= ESAPI.encoder().encodeForHTMLAttribute(contextPath) %>/js/browserVersion.js"></script>
<script language="JavaScript" src="<%= ESAPI.encoder().encodeForHTMLAttribute(contextPath) %>/js/auth.js"></script>
<script language="JavaScript">

    writeCSS('<%= ESAPI.encoder().encodeForJavaScript(contextPath) %>');
...
<body class="LogBdy" onload="placeCursorOnFirstElm();">
  ...
  <table border="0" cellpadding="0" cellspacing="0" align="center" title="">
    <tr>
      <td width="50%"><img src="<%= ESAPI.encoder().encodeForHTMLAttribute(contextPath) %>/images/dot.gif" width="1" height="1" alt="" /></td>
      <td><img src="<%= ESAPI.encoder().encodeForHTMLAttribute(contextPath) %>/images/dot.gif" width="728" height="1" alt="" /></td>
      <td width="50%"><img src="<%= ESAPI.encoder().encodeForHTMLAttribute(contextPath) %>/images/dot.gif" width="1" height="1" alt="" /></td>
    </tr>
    <tr class="LogTopBnd" style="background-image: url(<%= ESAPI.encoder().encodeForHTMLAttribute(ESAPI.encoder().encodeForCSS(contextPath)) %>/images/gradlogtop.jpg);
    background-repeat: repeat-x; background-position: left top;">
      <td><img src="<%= ESAPI.encoder().encodeForHTMLAttribute(contextPath) %>/images/dot.gif" width="1" height="30" alt="" /></td>
      <td>&nbsp;</td>
    </tr>
    <tr>
      <td class="LogMidBnd" style="background-image: url(<%= ESAPI.encoder().encodeForHTMLAttribute(ESAPI.encoder().encodeForCSS(contextPath)) %>/images/gradlogsides.jpg);
        background-repeat:repeat-x;background-position:left top;">&nbsp;</td>
      <td class="LogCntTd" style="background-image: url(<%= ESAPI.encoder().encodeForHTMLAttribute(ESAPI.encoder().encodeForCSS(contextPath)) %>/images/login-backimage-long.jpg);
        background-repeat:no-repeat;background-position:left top;" height="435" align="center" valign="middle">
        <table border="0" background="<%= ESAPI.encoder().encodeForHTMLAttribute(contextPath) %>/images/dot.gif" cellpadding="0" cellspacing="0"
        width="100%" title="">
          <tr>
            <td width="260"><img src="<%= ESAPI.encoder().encodeForHTMLAttribute(contextPath) %>/images/dot.gif" width="260" height="245" alt="" /></td>
            <td width="415" bgcolor="#ffffff" valign="top">
        <form name="realm_form" action="<%=ESAPI.encoder().encodeForHTMLAttribute(request.getRequestURI())%>"
            onSubmit="formSubmit(); return false;" method="post">
              <table border="0" cellspacing="0" cellpadding="0">
                <tr>
                  <td colspan="2">
		      <img src="<%= ESAPI.encoder().encodeForHTMLAttribute(contextPath) %>/images/dot.gif" width="1" height="25" alt="" />
		  </td>
                </tr>
	...
	<td><div class="logInp">
        ...
        <input type="hidden" name="wreply" value="<%=ESAPI.encoder().encodeForHTMLAttribute(wreply)%>" />
<%
        if ( wctx != null && wctx.length() > 0 ) {
%>
        <input type="hidden" name="wctx" value="<%=ESAPI.encoder().encodeForHTMLAttribute(wctx)%>" />
<%
        }
%>
...
<%
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(jspFile + "Account Realm Cookie: " +
                ...
                metaManager.getAllRemoteIdentityProviderEntities(spRealm))
            {
                ...
                if ( metaManager.isTrustedProvider(spRealm,
                    ...
                    if (logger.isDebugEnabled()) {
                        logger.debug(jspFile + "account realm key: " +
...
%>
                    <option value="<%=ESAPI.encoder().encodeForHTMLAttribute(issuerName)%>"
                        <%=((accountRealmCookieValue != null) &&
                        (accountRealmCookieValue.equals(issuerName))?"selected":"")%>>
                        <%=ESAPI.encoder().encodeForHTML(displayName)%>
                    </option>
<%
                }
            }
%>
	...
	<!-- Submit button -->
	<tr>
	<td><img src="<%= ESAPI.encoder().encodeForHTMLAttribute(contextPath) %>/images/dot.gif"
	...
	<tr>
      ...
      <td width="45"><img src="<%= ESAPI.encoder().encodeForHTMLAttribute(contextPath) %>/images/dot.gif"
    ...
    <tr class="LogBotBnd" style="background-image: url(<%= ESAPI.encoder().encodeForHTMLAttribute(ESAPI.encoder().encodeForCSS(contextPath)) %>/images/gradlogbot.jpg);
    background-repeat:repeat-x;background-position:left top;">
      <td>&nbsp;</td>
      <td>
        <div class="logCpy"><span class="logTxtCpy">
          Copyright &copy; 2008-2021, ForgeRock AS. All Rights Reserved. Use of this software is subject to the terms and conditions of the ForgeRock&#8482; License and Subscription Agreement.
        </div>
      </td>
      <td>&nbsp;</td>
    </tr>
